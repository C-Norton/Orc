...
def register_health_commands(bot: commands.Bot) -> None:
    ...
    async def hp_damage(interaction: discord.Interaction, amount: str, partymember: str = None) -> None:
        ...
        try:
            ...
            msg = Strings.HP_DAMAGE_MSG.format(
                char_name=char.name, amount=dmg, current=char.current_hp, max=char.max_hp
            )
            if char.temp_hp > 0:
            ...
            logger.info(f"/hp damage: {char.name} took {dmg} damage, HP now {char.current_hp}/{char.max_hp}")
        finally:
            db.close()
    ...
    async def hp_heal(interaction: discord.Interaction, amount: str, partymember: str = None) -> None:
        ...
        try:
            ...
            await interaction.response.send_message(
                Strings.HP_HEAL_MSG.format(
                    char_name=char.name, amount=healing, current=char.current_hp, max=char.max_hp
                )
                + death_save_reset_msg
            )
            logger.info(f"/hp heal: {char.name} healed {healing}, HP now {char.current_hp}/{char.max_hp}")
        finally:
            db.close()

    @hp_group.command(name="temp", description="Add temporary HP to your active character")
    ...
    @hp_group.command(name="party_temp", description="Add temporary HP to all members of your active party")
    @app_commands.describe(amount="Temporary HP amount")
    async def hp_party_temp(interaction: discord.Interaction, amount: int) -> None:
        ...
        try:
            ...
            db.commit()
            logger.info(f"/hp party_temp: updated {len(lines)} characters in '{party.name}'")
