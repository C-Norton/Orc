...
def test_delete_character():
    ...
    try:
        ...
        char = (
            db.query(Character)
            .filter_by(user_id=user.id, server_id=server.id, name=char_name)
            .first()
        )
        if not char:
        ...
        assert (
            db.query(CharacterSkill).filter_by(character_id=char.id).first() is not None
        )
        ...
        target_char = (
            db.query(Character)
            .filter_by(user=user, server=server, name=char_name)
            .first()
        )
        ...
        target_char = (
            db.query(Character)
            .filter_by(user=user, server=server, name="Does Not Exist")
            .first()
        )
        ...
        found = (
            db.query(Character)
            .filter_by(user=user2, server=server, name="Owner Test")
            .first()
        )
