...
def test_scenarios():
    ...
    try:
        ...
        char1 = (
            db.query(Character)
            .filter_by(user_id=user1.id, server_id=server.id, name="Hero")
            .first()
        )
        if not char1:
            char1 = Character(
                name="Hero", user_id=user1.id, server_id=server.id, is_active=True
            )
            db.add(char1)

        char2 = (
            db.query(Character)
            .filter_by(user_id=user2.id, server_id=server.id, name="Hero")
            .first()
        )
        if not char2:
            char2 = Character(
                name="Hero", user_id=user2.id, server_id=server.id, is_active=True
            )
            db.add(char2)
        ...
        try:
            ...
            db.commit()
            print(
                "! FAILED: Should not allow same user to have two characters with the same name."
            )
        except Exception as e:
            db.rollback()
            print(
                f"✓ Correctly prevented same user from having two characters named 'Hero'."
            )
        ...
        attack1 = Attack(
            character_id=char1.id, name="Sword", hit_modifier=5, damage_formula="1d8+3"
        )
        db.add(attack1)
        db.commit()
        ...
        attack_name = "Sword"
        existing_attack = (
            db.query(Attack).filter_by(character_id=char1.id, name=attack_name).first()
        )
        if existing_attack:
        ...
        updated_attack = (
            db.query(Attack).filter_by(character_id=char1.id, name="Sword").first()
        )
        ...
        selected_char = (
            db.query(Character)
            .filter_by(name="Hero", server_id=server.id, user_id=owner.id)
            .first()
        )
        ...
        char_found = (
            db.query(Character)
            .filter_by(user_id=user1.id, server_id=server.id, name=target_name)
            .first()
        )
