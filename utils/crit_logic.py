...
def apply_crit_damage(formula: str, crit_rule: CritRule) -> CritResult:
    ...
    if not match:
        ...
        logger.warning(
            f"apply_crit_damage: unexpected formula '{formula}'; falling back to normal roll"
        )
        rolls, modifier, total = roll_dice(formula)
        return CritResult(
            rolls=rolls, modifier=modifier, total=total, grants_inspiration=False
        )
    ...
    if crit_rule == CritRule.DOUBLE_DICE:
        ...
        logger.debug(
            f"apply_crit_damage DOUBLE_DICE: {formula} → {crit_formula}, total={total}"
        )
        return CritResult(
            rolls=rolls, modifier=mod, total=total, grants_inspiration=False
        )
    ...
    if crit_rule == CritRule.DOUBLE_DAMAGE:
        ...
        logger.debug(
            f"apply_crit_damage DOUBLE_DAMAGE: {formula}, total={total} → {doubled_total}"
        )
        return CritResult(
            rolls=rolls, modifier=mod, total=doubled_total, grants_inspiration=False
        )
    ...
    if crit_rule == CritRule.PERKINS:
        rolls, mod, total = roll_dice(formula)
        logger.debug(
            f"apply_crit_damage PERKINS: {formula}, total={total} (inspiration granted)"
        )
        return CritResult(
            rolls=rolls, modifier=mod, total=total, grants_inspiration=True
        )
